...
plot_estado <- function(estado_tbl) {
  ...
  p <- ggplot2::ggplot(
    ...
    ggplot2::labs(
      ...
      subtitle = "Universo total de proyectos de la solapa Proyectos",
      x = NULL,
  ...
  plotly::ggplotly(p, tooltip = "text") |>
    plotly::layout(showlegend = FALSE, margin = list(l = 80, r = 30, t = 70, b = 45))
}
...
plot_bar_monto <- function(data, label_col, title, subtitle = NULL, top_n = NULL, fill_color = bar_color) {
  ...
  p <- ggplot2::ggplot(
    plot_data,
    ggplot2::aes(x = label_plot, y = monto_usd_mill, text = hover)
  ) +
...
plot_compare_total_aprobado <- function(total_tbl, aprobado_tbl, label_col, title, subtitle = NULL, top_n = 12) {
  ...
  total_data <- total_tbl |>
    dplyr::transmute(
      categoria = .data[[label_col]],
      universo = "Total",
      n_proyectos = n_proyectos,
      monto_usd_mill = monto_usd_mill
    )
  ...
  plot_data <- dplyr::bind_rows(total_data, aprobado_data) |>
    ...
    tidyr::complete(
      categoria,
      universo = c("Total", "Aprobados"),
      fill = list(n_proyectos = 0, monto_usd_mill = 0)
    ) |>
    dplyr::mutate(
  ...
  p <- ggplot2::ggplot(
    plot_data,
    ggplot2::aes(x = categoria_plot, y = monto_usd_mill, fill = universo, text = hover)
  ) +
...
plot_timeline <- function(timeline_tbl) {
  ...
  data <- timeline_tbl |>
    dplyr::mutate(
      ...
      size_plot = dplyr::case_when(
        is.na(monto_usd_mill) ~ 8,
        monto_usd_mill <= 0 ~ 8,
        TRUE ~ sqrt(monto_usd_mill)
      ),
      hover = glue::glue(
        "Proyecto: {proyecto}<br>",
        "Evento: {evento}<br>",
  ...
  plotly::plot_ly(
    ...
    plotly::layout(
      title = list(text = "Línea de tiempo de presentaciones y aprobaciones"),
...
datatable_proyectos <- function(proyectos) {
  table_data <- proyectos |>
    dplyr::transmute(
      ...
      `Fecha de aprobación` = fmt_date(fecha_aprobacion),
  ...
  DT::datatable(
    ...
    DT::formatRound(
      columns = c("Monto (mill. USD)", "Activos computables (mill. USD)"),
      digits = 1,
      mark = ".",
      dec.mark = ","
    )
